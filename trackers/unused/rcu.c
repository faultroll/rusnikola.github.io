...
// using ssmem
/* DECLARE_THREAD_LOCAL(alloc, ssmem_alloc_t *);

void rcu_thread_init(int id) {
    LOCALIZE_THREAD_LOCAL(alloc, ssmem_alloc_t *);

    alloc = (ssmem_alloc_t *) nbd_malloc(sizeof (ssmem_alloc_t));
    assert(alloc != NULL);
    ssmem_alloc_init(alloc, SSMEM_DEFAULT_MEM_SIZE, id);
}

void *rcu_ref_alloc(size_t n) {
    LOCALIZE_THREAD_LOCAL(alloc, ssmem_alloc_t *);

    return ssmem_alloc(alloc, n);
}
